...
A local route also carries the declared dependencies of `config/crew-dispatch.json`: each git-excluded `.agents/skills/<policy>/` directory that a `policy-service` rule or default names, converged as a whole tree so an inherited rule reaches a home that can actually load its policy.
A policy skill tracked in the repository is left to the tracked-files fast-forward, and a remote route carries only the file allowlist, so any home that still cannot load a named policy reports it at its own session start as a `CREW_DISPATCH:` diagnostic and refuses to select around it.
