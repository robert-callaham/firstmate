...
When a matched rule or default is a profile array, its selector decides who chooses among the candidates (`AGENTS.md` section 4 owns that boundary): load `quota-array-dispatch` for the built-in `quota-balanced` selector after this skill establishes harness and model/provider facts, or the rule's named `policy` skill for `select: "policy-service"`.
...
A guarded silent hook cannot be verified from absence of effect, so never conclude from a missing `state/<id>.turn-ended` that the hook did not fire.
Apply [`bin/fm-brief.sh`](../../../bin/fm-brief.sh)'s generated `GUARD_SELF_REPORTING_RULE` contract to this silent hook; its positive control is an unguarded invocation probe.
