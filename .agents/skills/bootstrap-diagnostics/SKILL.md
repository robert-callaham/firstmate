...
- `CREW_DISPATCH: invalid config/crew-dispatch.json - <reason>` - the optional dispatch profile file exists but failed low-cost bootstrap validation; stop profile-based dispatch, report the actionable error, and require correction of the malformed schema, unverified harness name, orphaned selector or policy field, or invalid harness/effort pair rather than falling back around it or selecting a bad profile.
- `CREW_DISPATCH: policy skill not installed in this home - <name> (expected .agents/skills/<name>/SKILL.md); ...` - the file is valid but a `policy-service` selector names a skill this home cannot load, so every rule naming it is blocked; report the named skill and require that the captain install that declared local policy dependency here or correct the configured policy name.
  This home may be a secondmate that inherited the rule while propagation skipped the skill, so name the home too; never select around the blocked rule with another selector, another candidate, or `config/crew-harness`.
