...
The default `quota-balanced` selector keeps that decision inside firstmate through the `quota-array-dispatch` procedure over current quota output, while `select: "policy-service"` (or `default_select` for the top-level default) delegates the complete array plus the one shared quota snapshot acquired for that intake to the local skill named by the rule's `policy` field and accepts only its selected concrete profile or its explicit no-route.
Those named policy skills are git-excluded local material, so they propagate into local secondmate homes with the dispatch file and each receiving home reports an unresolvable policy at its own session start rather than falling back.
...
The session-start bootstrap step keeps schema-valid dispatch configuration silent unless verbose facts are enabled and surfaces a concise invalid-config line when validation fails.
It also prints one separate non-invalid diagnostic, without verbose facts, when a schema-valid file names a `policy-service` owner that resolves to no local policy skill, because that intake would otherwise dead-end where the contract forbids a silent fallback.
...
The helper requires a full `https://<host>/<owner>/<repo>/pull/<n>` URL, invokes `gh-axi pr merge <n> --repo <owner>/<repo> --hostname <host>`, defaults to `--squash`, preserves explicit merge-method flags, and rejects malformed URLs or repo/host override flags before recording merge state; a well-formed GitLab merge request URL (see [docs/gitlab-merge-watch.md](gitlab-merge-watch.md)) is refused too, explicitly, rather than sent to the wrong forge.
`<host>` is `github.com` or a GitHub Enterprise Server hostname declared in `config/github-hosts` (one lowercase DNS name per line, `#` comments ignored).
That allowlist is a security boundary, not a convenience: it is what replaced the old hardcoded `github.com` and it is the only thing refusing lookalike hosts such as `github.com.evil`, `evilgithub.com`, or a punycode homograph, any of which is a valid DNS name that would otherwise arm a watch or aim a merge at another forge.
With no such file, behavior is identical to the `github.com`-only pin.
