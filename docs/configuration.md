...
Resolution applies to reading a home's own value; propagation refuses any inherited destination that is a symlink whose target resolves, reporting a concrete error and leaving the link untouched rather than replacing it, writing through it, or unlinking it to mirror an absent primary value.
That refusal covers every item in the inheritance contract, not just this one, so an operator can link an individual config item into a private tree without a convergence silently disconnecting it.
A dangling destination link is the opposite case and is deliberately not protected: it guards no tree, so it is replaced or removed like any other stale artifact rather than left to fail every convergence until someone removes it by hand.
A link counts as dangling only when its target is positively confirmed absent, meaning the target's own parent directory is present and searchable.
A link into an unmounted volume, a locked encrypted mount, or an unreadable directory is indeterminate rather than dangling, and is preserved with its own distinct diagnostic, because that state is indistinguishable from a live link by presence alone and session-start convergence runs early at login when exactly those are still unavailable.
Resolution of a symlinked `config/` directory is a read-path capability, which is where the operator layout above lives: a primary reads its own linked `config/` normally.
A secondmate destination is stricter and does not converge through a symlinked `config/` at all: one resolving outside its home is skipped as an unsafe home, and one resolving inside it is skipped because `git check-ignore` refuses a pathspec that traverses a symlink, so the gitignore guard reads the resulting non-zero exit as "not ignored" rather than ever evaluating the pattern.
Both outcomes are reported and leave the link untouched, which is the invariant that matters here: no propagation path removes or writes through an operator's live link.
