...
Resolution applies to reading a home's own value; propagation refuses any inherited destination that is itself a symlink, reporting a concrete error and leaving the link untouched rather than replacing it, writing through it, or unlinking it to mirror an absent primary value.
...
A secondmate destination is stricter and does not converge through a symlinked `config/` at all: one resolving outside its home is skipped as an unsafe home, and one resolving inside it is skipped because the gitignore guard's `config/` pattern matches directories rather than links.
Both outcomes are reported and leave the link untouched, which is the invariant that matters here: no propagation path removes or writes through an operator's link.
