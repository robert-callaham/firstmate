...
Resolution is a read-path capability, which is where the operator layout above lives: a home reads its own linked `config/` or linked file normally.
Propagation into a secondmate home is stricter and never replaces, writes through, or unlinks an operator's live destination link, and it does not converge an item through a symlinked destination `config/` at all; the inherited-local-material contract in [`secondmate-provisioning`](../.agents/skills/secondmate-provisioning/SKILL.md) owns which destination links count as dangling and what each case reports.
