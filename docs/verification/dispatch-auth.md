...
Because it drives the contract rather than an isolated skill, it copies `AGENTS.md` into the lab home, omits `--no-context-files` so Pi loads it, and opens with a load probe that fails the run when that copy never reaches model context.
...
It closes with a negative control that removes exactly the `policy-service-delegation` sentinel region from the lab copy alone, proves the rest of the contract survives byte for byte, and requires the same intake to still reach a dispatch decision while no longer delegating, so the delegation cases cannot silently stop depending on the paragraph they guard.
Those sentinels in `AGENTS.md` are load-bearing for that control and must survive edits to the delegation contract; the control is also probabilistic, both because the model may reach the policy unprompted and because the section 2 layout table and the section 13 skill catalog still point at `policy-service` from outside the stripped region, so a persistent pass is the signal to investigate rather than proof.
Known limitation: omitting `--no-context-files` also admits the operator's user-global Pi context files, because the lab cannot redirect `HOME` where Pi keeps its credentials, so a result can depend on the machine.
Close it by validating the suite on a machine with Pi installed and adopting a narrower project-context-only switch if Pi offers one.
