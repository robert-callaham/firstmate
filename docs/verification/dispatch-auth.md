...
It closes with a negative control that strips every `policy-service` line from the lab copy alone and requires the same intake to stop delegating, so the delegation cases cannot silently stop depending on the paragraph they guard; that control is probabilistic, since the model may reach the policy unprompted, and a persistent pass is the signal to investigate rather than proof.
